...
class Actor:
    def __init__(self, env, ):
        ...
        with self.graph.as_default():
            ...
            self.conv2 = tf.layers.conv2d(inputs=self.bn1, filters=32, kernel_size=4, strides=(2, 2),
                                          padding="valid", activation=tf.nn.relu,
                                          kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d())
            ...
            self.falt_d = tf.contrib.layers.flatten(inputs=self.bn2)

            self.drop1 = tf.nn.dropout(self.falt_d, self.drop_prob)
            ...
            self.drop2 = tf.nn.dropout(self.bn3, self.drop_prob)
    ...
    def rollout_policy(self, timeSteps, episodeNumber,drop_prob,if_train):
        ...
        for time in range(timeSteps):
            ...
            action = self.choose_action(cur_x,drop_prob,if_train)
    ...
    def update_policy(self, advantage_vectors,drop_ratio = 1.0,if_train = True):
    ...
    def choose_action(self, state, drop_ratio = 1.0,if_train = True):
...
class Critic:
    ...
    def update_value_estimate(self,drop_ratio = 1.0,if_train = True):
        global replay_states, replay_actions, replay_rewards, replay_next_states, replay_return_from_states
            ...
            for i in range(total_batch):
                ...
                self.sess.run(self.optim,
                              feed_dict={self.state_input: batch_state_input, self.return_input: batch_return_input,
                                         self.drop_prob: drop_ratio,self.if_train:if_train})

    def get_advantage_vector(self, states, rewards, next_states,drop_ratio = 1.0, if_train = True):
    ...
    def learn(self):
        ...
        for i in range(self.max_episodes):
            ...
            if (i + 1) % self.episodes_before_update == 0:
                ...
                if update:
                    print("Updating")
                    self.actor.update_policy(advantage_vectors,drop_prob,if_train)
                    self.critic.update_value_estimate(drop_prob,if_train)
                else:
