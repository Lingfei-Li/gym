...
class Actor(object):
    def __init__(self, sess, n_features, n_actions, lr=0.001):
        self.sess = sess
        self.state = tf.placeholder(tf.float32, [None, 80, 80, 3], "state")
...
class Critic(object):
    def __init__(self, sess, n_features, lr=0.01):
        ...
        self.state_fc = tf.reshape(self.state, [self.batch_size, 80 * 80 * 3])
    ...
    for i_episode in range(MAX_EPISODE):
        ...
        while True:
            ...
            if done or t >= MAX_EP_STEPS:
                ...
                reward_record.append(ep_rs_sum)
                if 'running_reward' not in globals():
                    running_reward = ep_rs_sum
                else:
                    running_reward = running_reward * 0.95 + ep_rs_sum * 0.05
                print("episode:",i_episode, "  reward:", ep_rs_sum, "  mean:", sum(reward_record)/len(reward_record))
