...
class Actor:
    def __init__(self, env, ):
       ...
       #     self.biases = tf.Variable(tf.random_normal([self.action_space_n]))
            ...
            self.weights = {
                ...
                'out' : tf.Variable(np.random.randn(H, self.action_space_n) / np.sqrt(I), dtype=tf.float32)
            }
 ...
 #       episode_return_from_states = episode_np.tolist()
    ...
    def choose_action(self, state):
        ...
        action = np.random.choice([0, 1, 2, 3, 4, 5], 1, replace=True, p=softmax_out[0])[0]  # Sample action from prob density
        return action
 ...
 #       self.biases_slow['out'] = tf.identity(self.biases['out'])
    ...
    def update_value_estimate(self):
        ...
        for epoch in range(self.num_epochs):
            total_batch = min(divmod(np.ma.size(replay_states) ,batch_size)[0], 5)
