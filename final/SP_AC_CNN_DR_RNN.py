...
class Critic:
    ...
    def get_advantage_vector(self, states, rewards, next_states,drop_ratio = 1.0):
        ...
        for i in range(len(states)):
            ...
            reward = rewards[i]
            state_value, rstate1 = self.sess.run([self.value_pred, self.rnn_state], feed_dict={self.state_input: state, self.trainLength:1, self.batch_size:1, self.state_in:rstate, self.drop_prob: drop_ratio})
